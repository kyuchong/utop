...
#: 릴리스 시험인가 — `E61xx-V0001`(새) · `E61xx_V0001`(옛).
#: Jira 이슈를 덮는 시험이고, **요구사항에는 붙지 않는다.**
_V_RE = __import__("re").compile(r"[_-]V\d+$")


async def tc_upsert(tcid: str, data: dict) -> None:
    """시험 하나를 저장한다.

    **릴리스 시험(_V)은 요구사항에 못 붙는다**(합의: 완전 분리).

    화면에서 요구사항 칸을 감춰 두었지만 그건 가리개일 뿐이다 — 옛 화면,
    API 직접 호출, 예전에 붙여 둔 자료로 얼마든지 들어올 수 있다. 들어오면
    REQ-Coverage 의 커버리지 수치(요구사항별 TC 수·TC Map)에 릴리스 시험이
    섞여 「요구사항이 얼마나 덮였나」 가 거짓이 된다.

    그래서 **저장하는 문 한 곳에서** 비운다. 화면을 어떻게 뚫어도 여기를
    지나므로 새지 않는다.
    """
    if _V_RE.search(tcid or "") and (data.get("req_id") or ""):
        data = {**data, "req_id": ""}
...
async def plan_run_next_key(model: str) -> str:
    ...
    사이클 Key 가 모델그룹(E61xx-C0001)인 것과 일부러 다르다. 실행은 어느
