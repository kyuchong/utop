...
import { stepPptOn } from '@/components/tc/types'
  ...
  const tcs: LguTc[] = useMemo(() => {
    ...
    return items.map((it) => {
      ...
      return {
        ...
        // 200줄이 그대로 실렸었다.
        /* 결과서에 실을 줄은 **줄마다** 정한다(승인) — 표의 PPTX 칸.
           칸을 안 건드린 줄은 갈래 기본값을 따르므로, 설정 페이지가 있던
           때와 실리는 것이 같다. */
        steps: (it.steps ?? []).filter((st) => stepPptOn(st)).map((st) => {
