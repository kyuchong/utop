...
export interface TcQuery {
  ...
  var?: string
  /**
   * **표에서 뽑기** (iTest 의 Response Map).
   *
   * 정규식을 손으로 쓰라고 하면 아무도 안 쓴다(지적). 표로 나오는 출력은
   * 「어느 열의 값을, 어느 행에서」 만 고르면 된다 — 그 두 가지를 여기에
   * 적어 두면 실행할 때 표를 다시 읽어 그 칸을 꺼낸다. 자리(글자 수)가
   * 아니라 **열 이름**으로 찾으므로 장비가 칸 폭을 바꿔도 안 깨진다.
   */
  col?: string
  /** 행 고르기 — `Port=Te0/${i}` 처럼. 변수도 쓴다 */
  where?: string
  /** 행 고르기(번호) — 자료 줄 몇 번째. `${i}` 처럼 변수도 쓴다 */
  row?: string
}
...
export interface MeterRule {
   ...
   * 어느 줄을 볼까.
   *
   * 스무 갈래로 뿌리면 「합계가 얼마」 만으로는 못 잡는 시험이 많다.
   * 한 갈래만 죽어도 합계는 멀쩡해 보인다 — 그래서 「모든 줄이」 와
   * 「어느 한 줄이라도」 가 따로 있어야 한다.
   *
   *  sum  합계 · avg 평균 · max 가장 큰 줄 · min 가장 작은 줄
   *  each 모든 줄이 맞아야 · any 어느 한 줄이라도 맞으면
   *  숫자 그 줄 하나
   */
  scope?: 'sum' | 'avg' | 'max' | 'min' | 'each' | 'any'
  /**
   * 어느 줄들만 볼까 — `1-10` · `1,3,5` · `1-5,11-15`. 비우면 전부.
   *
   * 스무 갈래를 뿌려 놓고 「앞의 열 줄 합계」 를 보는 시험이 있다.
   * 포트별로 갈라 재거나, 한 묶음만 떼어 보는 것이다. 전체 합계와
   * 줄 하나 사이가 통째로 비어 있었다.
   */
  pick?: string
  /** 스트림 차례(0-기준). scope 가 없을 때만 본다 — 없으면 합계(옛 뜻) */
  idx?: number
  op: '>=' | '<=' | '==' | '!=' | '>' | '<' | 'between' | '~'
  value: number
  /** between 의 위끝 · `~`(오차 이내)의 허용 퍼센트 */
  value2?: number
  /**
   * 값 대신 **다른 칸**과 견줄 때.
   *
   * 「받은 것이 보낸 것과 같은가」 가 가장 흔한 판정인데, 보낸 개수는
   * 시험마다 달라서 숫자로 적어 둘 수가 없었다. 칸끼리 견주게 한다.
   */
  rhsField?: string
  /** 이 값에 붙일 변수 이름 (선택) */
  var?: string
...
export interface TcStep {
  ...
  head?: boolean
  /**
   * 결과서(PPTX)에 실을 줄인가(지시).
   *
   * 스무 스텝을 다 실으면 장표가 스무 장이 된다. 실제로 고객에게 보이는
   ...
   *  · `none` — 판정만. 스트레스 시험(장비가 버티나)에 맞다
   *  · `fail` — 깨진 회차 + 마지막 20 회만(기본). 대부분 이걸로 충분하다
   *  · `all`  — 전부. 회차가 적을 때만 쓴다
   *  반복(loop) 스텝에 붙는다. */
  roundKeep?: 'all' | 'fail' | 'none'
  /** PASS · FAIL · 빈 값(미실행) */
  status?: string
   ...
   */
  rules?: Array<{ t: 'has' | 'not' | 'table' | 'skip' | 'skipcol' | 'cmp' | 'hasline' | 'rowcount'; v: string; l?: string; op?: string; rhs?: string }>
  /**
...
export interface TcData {
  ...
  /**
   * 구성도 판에 올려 둔 장비와 그 자리.
   *
   * 배선이 없어도 장비를 먼저 올려 놓을 수 있어야 한다 — 랩을 그리는
   * 순서가 「장비를 놓고 → 선을 잇는다」 이기 때문이다. 옮겨 놓은 자리도
   * 함께 담는다. 자동으로 놓아 준 자리가 늘 맞을 리 없고, 옮긴 것이
   * 다시 열었을 때 그대로여야 제 것으로 읽힌다.
   */
  topoNodes?: Array<{ dev: string; x: number; y: number }>
  /**
   * 토폴로지 그림.
   *
