...
async function runOne(
  ...
  if (kind === 'instrument' && (step.meterAct || !String(step.cli ?? step.data ?? '').trim())) {
    ...
    if (!server) {
       ...
       * 두고도 플랜에서만 이 말이 나올 때 어디를 봐야 할지 모른다.
       * 실제로 그랬다 — TC 화면(브라우저)에서는 되는데 플랜(실행 서버)
  ...
  const walk = async (start: number, end: number): Promise<void> => {
    ...
    while (i < end) {
      ...
      if (kind === 'loop') {
         ...
         *   지금 가장 큰 플랜 6.6MB · DB 전체 19MB
         *
         * 100회 × 스텝 6개라도 220KB 다. 플랜 한 건이 이미 그 서른 배다.
