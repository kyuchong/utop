      ...
      <div className={`sd-body${readOnly ? ' sd-ro' : ''}`} inert={readOnly || undefined}>
        ...
        */}
        ...
        {/* 명령 뒤 **더 기다리는 시간**. 프롬프트가 돌아온 뒤에도 늦게 올라오는
            syslog 를 받으려는 대기다. 비워 두면 서버 기본값으로 도는데, 그때
            **명령마다 0.24초**가 그냥 나간다 — 조회만 하는 스텝은 0 으로 두면
            그만큼 빨라진다. reload 처럼 한참 뒤에 더 뱉는 명령에만 올린다.

            **label 밖에 두지 않는다** — 다른 줄과 같은 sd-f 격자를 쓰고,
        ...
        {kind === 'cli' && (
          <label className="sd-f">
            ...
            <span className="sd-wait">
              ...
              />
              <span className="sd-hint">
                초. 비우면 기본 — 명령마다 <b>0.24초</b>쯤 더 기다립니다. 조회만 하는 명령은{' '}
                <b>0</b> 이 빠릅니다. reload 처럼 늦게 더 뱉는 명령만 올리세요.
              </span>
            </span>
          </label>
        )}
