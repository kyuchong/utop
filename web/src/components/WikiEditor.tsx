...
const SLASH_ORDER: ReadonlyArray<readonly [string, readonly string[]]> = [
  ...
  [
    /* **표를 맨 앞에 둔다**(지적: 「/」 를 쳐도 표가 안 보인다). 「고급」 에 있어서
       메뉴를 한참 굴려야 닿았는데, 보고서·회의록에서 가장 자주 쓰는 블록이다.
       `/표` 로 바로 부를 수도 있지만, 그것을 알아야만 쓸 수 있는 것은 숨은 것이다. */
    '기본 블록',
    ['표', '데이터베이스', '번호 매기기 목록', '글머리 기호 목록', '체크리스트', '인용', '코드 블록', '페이지 나누기', '구분선', '상자'],
  ],
 ...
 */
function PdfRail({ url, at, onPick }: { url: string; at: number; onPick: (n: number) => void }) {
...
export default function WikiEditor({
  ...
  /** 미리보기가 짚고 있는 쪽 — 쪽 목록에서 고른다 */
  const [pvAt, setPvAt] = useState(1)
        ...
        >
          <div className="wke-pv" role="dialog" aria-modal="true" aria-label="PDF 미리보기">
            ...
            {navigator.pdfViewerEnabled === false ? (
              ...
              /* 쪽 목록은 **우리가 그린다**(PdfRail) — 내장 뷰어의 썸네일은
                 iframe 안에서 검은 띠로만 남는 크롬이 있다(지적). 본문은
                 내장 뷰어 그대로 — 내려받는 파일과 같은 바이트다.
